...
g1 <- ggplot(temp, aes(x = HST, y = FTHG)) + geom_smooth(method = loess) + geom_point(aes(size = Count, color = HC)) + labs(x = "Broj udaraca domacina na gol", y = "Broj zabijenih golova domacina", title = "Odnos udaraca na gol i postignutih golova domacina", size = "Broj slucajeva", color = "Broj slucajeva")

group_by(bl, AST, FTAG, AC) %>% summarise(Count = n()) -> temp
g2 <- ggplot(temp, aes(x = AST, y = FTAG)) + geom_smooth(method = loess) + geom_point(aes(size = Count, color = AC)) + labs(x = "Broj udaraca gosta na gol", y = "Broj zabijenih golova gosta", title = "Odnos udaraca na gol i postignutih golova gosta", size = "Broj slucajeva", color = "Broj slucajeva")
grid.arrange(g1, g2, nrow = 2, ncol = 1)
```
\pagebreak
